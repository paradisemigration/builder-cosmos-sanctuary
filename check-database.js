...
const db = new sqlite3.Database(DB_PATH, (err) => {
  ...
  db.get("SELECT COUNT(*) as count FROM businesses", (err, row) => {
    ...
    // Check cities
    db.get(
      "SELECT COUNT(DISTINCT city) as count FROM businesses",
      (err, row) => {
        if (err) {
        ...
        // Check images
        db.get(
          'SELECT COUNT(*) as count FROM businesses WHERE images IS NOT NULL AND images != ""',
          (err, row) => {
            if (err) {
              console.error("❌ Error counting images:", err.message);
            } else {
              console.log(
                `🖼️  Businesses with images: ${row.count.toLocaleString()}`,
              );
            }
              ...
              (err, rows) => {
                if (err) {
                  ...
                  rows.forEach((row, index) => {
                    console.log(
                      `   ${index + 1}. ${row.city}: ${row.count} businesses`,
                    );
                  });
                }
