...
function getCategorySpecificContent(categoryName: string): { description: string; keywords: string } {
  const categoryLower = categoryName.toLowerCase();

  if (categoryLower.includes('immigration consultant')) {
    return {
      description: 'Expert immigration lawyers and consultants for legal assistance, permanent residence applications, and citizenship guidance.',
      keywords: 'immigration lawyer, pr application, citizenship consultant, legal immigration services, immigration documentation'
    };
  } else if (categoryLower.includes('visa consultant')) {
    return {
      description: 'Professional visa consultants for all types of visa applications including tourist, business, student, and work visas.',
      keywords: 'visa agent, visa processing, tourist visa, business visa, visa documentation, visa application services'
    };
  } else if (categoryLower.includes('study abroad')) {
    return {
      description: 'Trusted study abroad consultants for international education guidance, university admissions, and student visa assistance.',
      keywords: 'overseas education, university admission, student visa, education consultant, international studies, abroad admission'
    };
  } else if (categoryLower.includes('work permit') || categoryLower.includes('work visa')) {
    return {
      description: 'Specialized consultants for work permits, employment visas, and job visa processing for international employment.',
      keywords: 'work permit, employment visa, job visa, work authorization, employment permit, international job visa'
    };
  } else if (categoryLower.includes('canada')) {
    return {
      description: 'Canada immigration specialists for Express Entry, Provincial Nominee Program, and Canadian permanent residence applications.',
      keywords: 'canada pr, express entry, pnp program, canada immigration, canadian visa, maple leaf card'
    };
  } else if (categoryLower.includes('australia')) {
    return {
      description: 'Australia immigration consultants for skilled migration, PR visas, and Australian work visa applications.',
      keywords: 'australia pr, skilled migration, australian visa, 189 visa, 190 visa, australia immigration'
    };
  } else if (categoryLower.includes('usa') || categoryLower.includes('america')) {
    return {
      description: 'USA visa consultants for H1B, L1, EB5, student visas, and American immigration services.',
      keywords: 'usa visa, h1b visa, l1 visa, eb5 visa, america immigration, us visa consultant'
    };
  } else if (categoryLower.includes('uk') || categoryLower.includes('britain')) {
    return {
      description: 'UK visa consultants for Tier 1, Tier 2, student visas, and British immigration services.',
      keywords: 'uk visa, tier 1 visa, tier 2 visa, britain immigration, uk immigration consultant'
    };
  } else if (categoryLower.includes('europe')) {
    return {
      description: 'European work visa specialists for Schengen visas, EU Blue Card, and European employment permits.',
      keywords: 'europe visa, schengen visa, eu blue card, european work permit, europe immigration'
    };
  } else if (categoryLower.includes('germany')) {
    return {
      description: 'Germany work permit and visa consultants for EU Blue Card, job seeker visa, and German immigration.',
      keywords: 'germany visa, eu blue card, germany work permit, job seeker visa, german immigration'
    };
  } else if (categoryLower.includes('norway')) {
    return {
      description: 'Norway work permit specialists for skilled worker visas and Norwegian immigration services.',
      keywords: 'norway work permit, norway visa, norwegian immigration, skilled worker visa norway'
    };
  } else if (categoryLower.includes('family visa')) {
    return {
      description: 'Family visa consultants for spouse visa, dependent visa, and family reunion immigration services.',
      keywords: 'family visa, spouse visa, dependent visa, family reunion, family immigration'
    };
  } else if (categoryLower.includes('tourist') || categoryLower.includes('visit')) {
    return {
      description: 'Tourist and visit visa services for leisure travel, family visits, and short-term stay visas.',
      keywords: 'tourist visa, visit visa, travel visa, holiday visa, short term visa'
    };
  } else if (categoryLower.includes('business visa')) {
    return {
      description: 'Business visa consultants for investor visas, business travel, and commercial visit permits.',
      keywords: 'business visa, investor visa, commercial visa, business travel visa, entrepreneur visa'
    };
  } else if (categoryLower.includes('golden visa')) {
    return {
      description: 'Golden visa and investor visa consultants for long-term residence through investment programs.',
      keywords: 'golden visa, investor visa, investment visa, residence by investment, citizenship by investment'
    };
  }

  // Default fallback
  return {
    description: 'Professional visa and immigration consultation services with expert guidance and documentation support.',
    keywords: 'visa services, immigration services, documentation support, visa guidance, immigration consultant'
  };
}
