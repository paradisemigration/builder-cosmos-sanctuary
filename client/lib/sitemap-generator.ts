...
export function setupSEOCrawling(): void {
  ...
  if (!document.querySelector('meta[name="google-site-verification"]')) {
    ...
    googleVerification.setAttribute(
      "content",
  ...
  if (!document.querySelector('meta[name="msvalidate.01"]')) {
    ...
    bingVerification.setAttribute(
      "content",
      "your-bing-verification-code-here",
    );
  ...
  const geoMetas = [
    ...
    {
      name: "DC.title",
      content: "VisaConsult India - Find Top Visa Consultants",
    },
  ];
...
export function generateImageSitemap(): string {
  ...
  allCities.slice(0, 10).forEach((city) => {
    // Limit to first 10 cities for performance
